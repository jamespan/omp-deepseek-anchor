# Anchored Standard for omp — 操作文档

把 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard) 的两阶段锚定移植到 omp，用于 DeepSeek V4 Pro。本文档自包含：照做即可复现全部配置。
...
- 一个 DeepSeek API key（官方端点 `api.deepseek.com`，需支持 `/v1/responses`；本项目实测可用）
- 目标模型 id 为 `deepseek-v4-pro`（其他模型不受影响）

## 第一步：安装扩展

克隆/下载本仓库后，用 omp 的插件机制安装（会自动加载 `package.json` 里声明的 `omp.extensions`）：

```sh
omp install /path/to/omp-deepseek-anchor     # 本地目录（link 方式）
omp install npm:omp-deepseek-anchor          # 发布到 npm 后
omp plugin list                              # 确认已安装
```

手动兜底：把本仓库的 `index.ts` 复制为 `~/.omp/agent/extensions/anchored.ts`（完整内容见下）：
...
- 删掉 `~/.omp/agent/extensions/anchored.ts`。
